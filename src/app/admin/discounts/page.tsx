...
export default function AdminDiscountsPage() {
  ...
  const fetchDealers = async () => {
    ...
    try {
      ...
      console.error('Error fetching dealers for discounts:', err);
      setError(err.response?.data?.error || 'Failed to fetch dealers for discount management.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div className="container mx-auto px-4 py-8">Loading dealers for discount management...</div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Discount Management</h1>
      
      {dealers.length === 0 ? (
        <p className="text-gray-700">No dealers found. Please create dealers first to manage discounts.</p>
      ) : (
              ...
              {dealers.map((dealer) => (
                <tr key={dealer.id}>
                  ...
                  <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                    <Link href={`/admin/discounts/${dealer.id}`} className="text-blue-600 hover:text-blue-900">Manage Discounts</Link>
                  </td>
                </tr>
              ))}
            </tbody>
